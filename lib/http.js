...
export const actionFrc = (url, action, datos, type) =>  {
  return async (_, getState) => {
    try {
      ...
      console.log(url, action, data, type)
...
export const getFrc = (url, active) =>  {
  return async dispatch => {
    try {
      let data
      if (active) {
        const { data: response } = await http.get(`${url}/getallactive`)
        data = response
      } else {
        const { data: response } = await http.get(`${url}/getall`)
        data = response
      }
      if (url === 'fcrmailshot') {
