...
class BlogHome extends Component {
	render() {
		...
		let entries = news.data.length
			? news.data.filter((e) => e.isPublic === true && e.active === true && e.important)
		...
		return (
			<section className={[ seccion, m0 ].join(' ')}>
				<Grid container className={container}>
					<Grid item xs={12}>
						<h3 className={[ h3, textCenter ].join(' ')}>
					...
					<Grid item xs={12}>
						<Grid spacing={24} container>
							{entries.length ? (
								entries.slice(0, 8).map((show, _, array) => {
														...
														<Link href={`/noticias?id=${show.id}`}>
															<a style={{ textDecoration: 'none' }}>
																<span style={{ color: '#cf8100', fontSize: 12 }}>
																	<Icon style={{ fontSize: 14 }}>event_note</Icon>{' '}
...
export default connect(mapStateToProps, mapDispatchToProps)(withStyles(STYLES, { name: 'WiBlogHome' })(BlogHome));
