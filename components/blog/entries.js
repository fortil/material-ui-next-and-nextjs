...
import { sanitizeHtml, sortArrayByDate, formatDateGuion } from '../../lib/utils'
...
class BlogNews extends Component {
  ...
  render() {
    ...
    let entries = news.data.length ? news.data
      .filter(e => e.isPublic === true && e.active === true)
      : []
    ...
    return (
      <section className={[seccion, m0].join(' ')}>
        <Grid container className={container}>
          <Grid item xs={12}>
            <h3 className={[h3, textCenter].join(' ')} style={{ textTransform: 'uppercase' }}><strong>Noticias</strong></h3>
          </Grid>
          <Grid item xs={12} className={classes.mobileCss}>
            <Grid spacing={24} container className={classes.mobileCss}>
              {entries.length ? entries.map((show, _, array) => {
                const image = show.imageUrl ? show.imageUrl : `../../static/logo.svg`
                  ...
                  <Grid item md={md} key={show.id} className={classes.hover}>
                    ...
                    <div className={[mt3, mb4].join(' ')} style={{ maxWidth: '100vw' }}>
                      <article style={{ display: 'block' }}>
                        ...
                        <Link href={`/noticias?id=${show.id}`}><p style={{ fontSize: 12 }}>{`${sanitizeHtml('abstract', show.description)}`}</p></Link>
                        <div className={postMeta}>
                          <Link href={`/noticias?id=${show.id}`}><a style={{ textDecoration: 'none' }}><span style={{ color: '#cf8100', fontSize: 12 }}><Icon style={{ fontSize: 13 }}>event_note</Icon> {show.creationDate.split(' ')[0]}</span></a></Link>
                        </div>
                      </article>
                    </div>
                  </Grid>
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withStyles(STYLES, { name: 'WiBlogNews' })(BlogNews))
