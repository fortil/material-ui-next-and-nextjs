			...
			<article className={[ seccion, m0, p0 ].join(' ')}>
				<Grid container className={[ container, classes.mobileCss ].join(' ')}>
					<Grid item md={8} className={classes.mobileCss}>
						<Grid
							container
							className={[ container, classes.mobileCss ].join(' ')}
							style={{ marginTop: 40 }}
						>
							<Grid item xs={12} className={classes.mobileCss}>
								<h3
									className={[ h3, textCenter, title ].join(' ')}
									style={{ textTransform: 'uppercase' }}
								>
									<strong>{this.state.entry.title}</strong>
								</h3>
							</Grid>
                ...
                {this.state.entry.youtube && this.state.entry.youtube !== 'false' ? (
								...
								{imageUrl ? (
									<img
										...
										alt={this.state.entry.title}
									/>
								) : (
									''
								)}
								<div
									className={[ textStyle, classes.marginB ].join(' ')}
									dangerouslySetInnerHTML={{ __html: this.state.entry.description }}
								/>
							</Grid>
						</Grid>
					</Grid>
						...
						{entries.filter((e) => e.isPublic === true && e.active === true).slice(0, 8).map((show) => {
							...
							return (
								<Grid
									...
									key={show.id}
									style={{ margin: 0 }}
									className={[ classes.mobileCss, classes.hover ].join(' ')}
								>
									<Grid item md={4}>
									...
									<Grid item md={8}>
										<div className={[ m0 ].join(' ')}>
											<article style={{ display: 'block' }}>
												...
												<p>{sanitizeHtml('abstract', show.description)}</p>
												<div className={postMeta}>
													<span style={{ color: '#cf8100' }}>
														<Icon>event_note</Icon> {show.creationDate.split(' ')[0]}
													</span>
												</div>
											</article>
										</div>
...
export default connect(mapStateToProps, mapDispatchToProps)(withStyles(STYLES, { name: 'WiEntry' })(Entry));
