				...
				<TableRow>
					<TableCell padding="checkbox">
						<Tooltip title="Acción" enterDelay={300}>
							<TableSortLabel>Acción</TableSortLabel>
						</Tooltip>
					</TableCell>
...
ProveedoresTableToolbar = connect(null, ProveedorestchEnhancedTableToolbar)(
	withStyles(toolbarStyles)(ProveedoresTableToolbar)
);
...
class ProveedoresTable extends React.Component {
	...
	handleClick = (action, provider) => {
		...
		if (action === 'ver') {
			Router.push(
				`/admin?page=providers&view=single&params=${JSON.stringify({ id: provider.id })}`,
				`/admin/providers/view/${provider.id}`
			);
		} else {
			swal({
										...
										>
											<TableCell padding="checkbox">
												<Tooltip title="Ver proveedor">
													<IconButton onClick={() => this.handleClick('ver', n)}>
														<Icon style={{ fontSize: 16 }}>visibility</Icon>
													</IconButton>
												</Tooltip>
												<Tooltip title={this.state.active === true ? 'Eliminar' : 'Activar'}>
													<IconButton onClick={() => this.handleClick('activate', n)}>
														<Icon style={{ fontSize: 16 }}>
															{this.state.active === true ? 'delete_outline' : 'publish'}
														</Icon>
													</IconButton>
												</Tooltip>
											</TableCell>
											<TableCell
												...
												onClick={() => this.handleClick('ver', n)}
											>
												{n.name}
											</TableCell>
											<TableCell
