...
class FormAdmin extends React.Component {
	...
	handleChange = (prop) => (event) => {
		...
		if (this.state[prop].hasOwnProperty('validate')) {
			error = this.state[prop].validate.reduce((prev, validateFn) => !validateFn(value) || prev, false);
		}
		this.setState({
	...
	createAction = () => {
      ...
      if (input.hasOwnProperty('required') && input.required === false) {
        return false
      }
      ...
      const keyActions = Object.keys(this.props.actions)
      const actions = []
      for (let i = 0; i < keyActions.length; i++) {
        actions.push(this.props.actions[keyActions[i]](inputsToCreate))
      }
      Promise.all(actions)
				.then(() => {
			...
			<div className={classes.root}>
				<Card className={classes.card}>
					<CardContent>
								...
								if (input.type === 'file') {
									return (
										<Grid
											...
											item
											md={md}
											className={[ classes.mt4, classes.flex, classes.justifyContentCenter ].join(
												' '
											)}
										>
											<input
											...
											<label htmlFor="raised-button-file">
												<div style={{ display: 'flex', flexFlow: 'column wrap', fontSize: 13 }}>
													...
													{this.state[input.name].value ? (
														<small className={classes.mt3}>
															{this.state[input.name].value.name}
														</small>
													) : (
														''
													)}
									...
									return (
										...
										>
											...
											>
												...
												{input.choices && input.choices.length ? (
													input.choices.map((option, i) => (
                      ...
                      <a
                        id={input.name}
                        href={this.state[input.name].value}
                        style={{ fontSize: 24 }}
                      >
                        {input.name}
                      </a>
									...
									return (
										<Grid
											...
											style={{ display: 'flex', flexFlow: 'column wrap' }}
										>
											<InputLabel style={{ fontSize: 13 }} htmlFor={input.name}>
